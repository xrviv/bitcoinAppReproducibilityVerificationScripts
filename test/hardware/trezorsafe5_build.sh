#!/bin/bash
# trezorsafe5_build.sh v0.1.1 - WalletScrutiny verification script for Trezor Safe 5 (T3T1)
# Organization: WalletScrutiny.com
# Last modified by: Bob (WalletScrutiny verification agent)
# Date last modified: 2026-09-10
# Usage: trezorsafe5_build.sh --version VERSION [--type TYPE] [--binary PATH] [--arch ARCH]
#
# Verifies Trezor Safe 5 (model T3T1) firmware reproducibility: builds from source via
# Trezor's upstream build-docker.sh at tag core/vVERSION, then compares the locally-built
# unsigned firmware against the official binary with its 65-byte firmware-header signature
# zeroed. Structurally a same-model-family fork of trezorsafe7_build.sh (T3W1) - both share
# the classic TRZV/TRZF vendor+firmware header format (python/src/trezorlib/firmware/core.py:
# FirmwareHeader, 1024B vendor header + 959B into a 1024B firmware header = offset 1983,
# 65B = 1 sigmask byte + 64 signature bytes), confirmed byte-for-byte against a downloaded
# Safe 5 v2.12.4 official binary before this script was written - see changelog.
#
# Changelog:
#   v0.1.1 (2026-09-10): build with `--targets firmware`. build-docker.sh's default target
#     list also builds a security monitor (secmon), which T3T1 does not have; since
#     core/v2.12.2 that step fails ("Sector address mismatch") before firmware.bin exists.
#     Upstream: https://github.com/trezor/trezor-firmware/issues/7775. Also pre-create the
#     build/core-T3T1* bind-mount directories so rootless podman can start the container.
#   v0.1.0 (2026-09-10): initial version, forked from trezorsafe7_build.sh v2.2.3.
...
handle_err() {
  ...
  write_results "ftbfs" "Trezor Safe 5 v${version:-?} (${firmwareType}): unexpected error (exit ${rc}) during verification."
  exit "$EXIT_FAIL"
}
trap handle_err ERR
...
usage() {
  echo 'NAME
       trezorsafe5_build.sh - verify Trezor Safe 5 (T3T1) hardware wallet firmware

SYNOPSIS
       trezorsafe5_build.sh --version VERSION [--type TYPE] [--binary PATH] [--arch ARCH]

DESCRIPTION
       --version   Firmware version to verify, e.g. "2.12.4" (required)
...
EXAMPLES
       trezorsafe5_build.sh --version 2.12.4 --type bitcoin-only
       trezorsafe5_build.sh --version 2.12.4 --type universal'
}
...
if command -v docker &>/dev/null && docker info &>/dev/null; then
  ...
  write_results "ftbfs" "Trezor Safe 5: docker daemon not responding and podman unavailable."
  exit "$EXIT_FAIL"
else
  echo -e "${RED}Error: neither docker nor podman found. Install one of them.${NC}"
  write_results "ftbfs" "Trezor Safe 5: no container runtime (docker/podman) available on host."
  exit "$EXIT_FAIL"
fi
...
if [[ ! "$version" =~ ^[0-9]+(\.[0-9]+)+$ ]]; then
  echo -e "${RED}Error: invalid --version '$version' (expected numeric like 2.12.4)${NC}"
  exit "$EXIT_INVALID"
fi
...
# (e.g. 2.12.4.7) is NOT silently built/compared as 2.12.4 while the report claims 2.12.4.7.
...
workDir="$(pwd)/trezor-safe5-work_${version}_${firmwareType}_$$"

echo
echo "Verifying Trezor Safe 5 (T3T1) firmware v$version ($firmwareType)"
echo
...
prepare() {
  ...
  if ! git clone "$repo" trezor-firmware; then
    ...
    write_results "ftbfs" "Trezor Safe 5 v${version}: failed to clone ${repo}."
    exit "$EXIT_FAIL"
  fi
  ...
  if ! git checkout "core/v${normalizedVersion}"; then
    ...
    write_results "ftbfs" "Trezor Safe 5 v${version}: git tag core/v${normalizedVersion} not found."
    exit "$EXIT_FAIL"
  fi
...
download_official() {
  ...
  if [[ -n "$BINARY_PATH" ]]; then
    ...
    if [[ "$firmwareType" == "bitcoin-only" ]]; then
      url="https://data.trezor.io/firmware/t3t1/trezor-t3t1-${normalizedVersion}-bitcoinonly.bin"
    else
      url="https://data.trezor.io/firmware/t3t1/trezor-t3t1-${normalizedVersion}.bin"
    fi
    if ! wget -q -O official.bin "$url"; then
      echo -e "${RED}Failed to download official firmware: $url${NC}"
      write_results "ftbfs" "Trezor Safe 5 v${version} (${firmwareType}): failed to download official firmware from ${url}."
      exit "$EXIT_FAIL"
    fi
  fi
...
build_firmware() {
  ...
  # Note: builds the default board revision pinned in this tag's model.toml
  # (revE as of 2026-09); no HW_REVISION override is passed, matching upstream's
  # own default production build.

  # Type-scoped image/container name so ABS's concurrent standard + bitcoin-only
  # runs of the same version don't collide on upstream's version-only name.
  ...
  if [[ "$firmwareType" == "bitcoin-only" ]]; then
    ...
    builtFirmware="build/core-T3T1-bitcoinonly/firmware/firmware.bin"
    ...
    builtFirmware="build/core-T3T1/firmware/firmware.bin"
    echo "Building standard firmware for $buildModel..."
  fi

  # Only the firmware target: the default list (boardloader bootloader secmon firmware)
  # tries to compile a secmon that T3T1 lacks and dies with "Sector address mismatch"
  # since core/v2.12.2 (trezor-firmware#7775). Upstream CI builds firmware the same
  # way (--targets 'firmware'), and the firmware's kernel dependency chain only pulls
  # secmon in for models whose model.toml sets secmon = true.
  # build-docker.sh bind-mounts build/core-T3T1{,-bitcoinonly} without creating them;
  # Docker creates bind sources implicitly, rootless podman refuses ("statfs ... no such
  # file or directory"), so create them first.
  mkdir -p build/core-T3T1 build/core-T3T1-bitcoinonly
  if ! ./build-docker.sh --models "$buildModel" --targets firmware "$skipFlag" "core/v${normalizedVersion}"; then
    echo -e "${RED}Build failed!${NC}"
    write_results "ftbfs" "Trezor Safe 5 v${version} (${firmwareType}): build-docker.sh build failed."
    exit "$EXIT_FAIL"
  fi

  if [[ ! -f "$builtFirmware" ]]; then
    echo -e "${RED}Built firmware not found: $builtFirmware${NC}"
    write_results "ftbfs" "Trezor Safe 5 v${version}: build completed but firmware not found at ${builtFirmware}."
    exit "$EXIT_FAIL"
  fi
...
compare_firmware() {
  ...
  # that follows -> offset 1024 + 959 = 1983. This is the shared classic FirmwareHeader
  # layout used across "Trezor core" models (python/src/trezorlib/firmware/core.py), not
  # a T3T1-specific constant - confirmed against a downloaded Safe 5 v2.12.4 binary (TRZV/
  # 1024B, then TRZF at offset 1024) before this script was written. Refuse to guess if the
  ...
  if [[ "$magicHex" != "54525a56" || "$vhLen" != "1024" ]]; then
    ...
    write_results "ftbfs" "Trezor Safe 5 v${version} (${firmwareType}): unexpected firmware header (magic=0x${magicHex}, vendor header=${vhLen}B); expected TRZV / 1024B for T3T1 - refusing to guess signature offset."
    exit "$EXIT_FAIL"
  fi
...
# ---- Main -------------------------------------------------------------------
echo "Starting Trezor Safe 5 verification..."
...
if [[ "$verdict" == "reproducible" ]]; then
  write_results "reproducible" "Trezor Safe 5 v${version} (${firmwareType}) reproducible from source via build-docker.sh at core/v${normalizedVersion} (commit ${commit}); official firmware-header signature (offset ${seekSize}, 65B) zeroed before SHA-256 comparison."
  echo
  echo "Trezor Safe 5 verification finished!"
  exit "$EXIT_OK"
else
  write_results "not_reproducible" "Trezor Safe 5 v${version} (${firmwareType}) not reproducible: built ${builtHash} != zeroed official ${officialZeroedHash} (signature zeroed at offset ${seekSize}, 65B)."
  echo
  echo "Trezor Safe 5 verification finished!"
  exit "$EXIT_FAIL"
fi
