...
#   uploader (passportprime_upload.sh). For each official Sparrow desktop artifact it
#   (1) uploads the file to a Blossom mediaserver and (2) publishes ONE NIP-94 file-metadata
#   event (kind 1063) per artifact, anchoring the file by its SHA256.
#
...
#   The WHOLE official release file, unmodified. Desktop artifacts (tar.gz/deb/rpm/zip/msi)
#   have no detached signature header to strip (unlike KeyOS firmware), so the Blossom
#   address EQUALS the SHA256 published on the GitHub release / shown on the WS page. NOTE:
#   the outer package hash is the *download* hash, not the reproducibility verdict - the
#   verdict comes from the build script's file-by-file content comparison (per Leo's model).
#
# IDENTITY
#   Uses a dedicated Nostr keypair (generated fresh on first run, persisted, and printed to
#   the terminal). Override the location with WS_UPLOAD_KEYFILE.
#
# SAFETY
...
RELEASES_REPO="sparrowwallet/sparrow"
...
# passportprime, future ones). Override per-run with WS_UPLOAD_KEYFILE.
...
usage() {
    cat <<EOF
sparrow_uploader.sh ${SCRIPT_VERSION} - upload + register Sparrow Desktop artifacts

Usage:
  $0 --version VERSION [--type LIST] [--publish] [--server URL] [--keyfile PATH]

  --version VERSION   Sparrow version without 'v' (e.g. 2.5.2). Required.
  --type LIST         Comma-separated artifact types, or 'all'. Default: all.
                      Valid: tarball, deb, rpm, zip, msi
                      (tarball/deb/rpm = Linux x86_64; zip/msi = Windows)
...
ART_FILE[tarball]="sparrowwallet-${V}-x86_64.tar.gz";   ART_MIME[tarball]="application/gzip"
ART_FILE[deb]="sparrowwallet_${V}-1_amd64.deb";          ART_MIME[deb]="application/vnd.debian.binary-package"
ART_FILE[rpm]="sparrowwallet-${V}-1.x86_64.rpm";         ART_MIME[rpm]="application/x-rpm"
ART_FILE[zip]="Sparrow-${V}.zip";                        ART_MIME[zip]="application/zip"
ART_FILE[msi]="Sparrow-${V}.msi";                        ART_MIME[msi]="application/x-msi"

# ---- resolve requested types ----
declare -a TYPES=()
if [[ "${TYPES_ARG}" == "all" ]]; then
    TYPES=(tarball deb rpm zip msi)
else
    IFS=',' read -ra TYPES <<< "${TYPES_ARG}"
fi
for t in "${TYPES[@]}"; do
    [[ -n "${ART_FILE[$t]:-}" ]] || die "unknown --type '${t}' (valid: tarball deb rpm zip msi)"
done
...
WORK="$(mktemp -d -t sparrowupload.XXXXXX)"
